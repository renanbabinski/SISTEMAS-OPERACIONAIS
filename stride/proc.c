...
userinit(void)
{
  ...
  p->tickets = MAX_TICKETS; //####O INIT CODE TAMBÉM PRECISA DE BILHETES, PEGA O MÁXIMO.####CHANGED
  p->stride = 0;
...
int
fork(int numtickets)  //antes fork era void, agora recebe um numero inteiro de bilhetes do processo
{
  int i, pid;
  struct proc *np;
  ...
  *np->tf = *proc->tf;
  ...
  // OK, não é necessário alterar para o stride
...
// ---------------------GERAR NUMERO ALEATÓRIO --------------------   #CHANGED
...
scheduler(void)
{
  ...
  int total_tickets;
  int process_id;
  //int chosen=-1;   //escolhido #CHANGED
  ...
  for(;;){
    ...
    //se não há bilhetes disponiveis, nao há ganhador
    total_tickets=lotteryTotal();
    process_id = smallest_step();


    if(total_tickets > 0){   //#CHANGED
      //Encontra o ganhador pela função LCG aleatória
     ...
     // }
      ...
      //if(total_tickets < chosen){
        //chosen %= total_tickets;   //Escolhido esta no intervalo de bilhetes
     // }/
      cprintf("ESCOLHIDO : %d\n", process_id);
      // Itera sobre a tabela de processos buscando o processo para rodar
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
        if(p->pid != process_id){
        	continue;
        }
	// Troca para o processo escolhido. É tarefa do processo liberar a trava
	// da ptable.lock e depois recupera-la antes de voltar para nós
        ...
        switchkvm();
        // O processo será executado por enquanto
	// Deveria ter mudado o estado antes de voltar.
        proc = 0;
      }
    }
